...
const { data: post } = await useAsyncData(`post-${route.params.id}`, () => fetchPost(route.params.id));
</script>
```
...
const [{ data: user }, { data: posts }] = await Promise.all([useFetch("/api/user"), useFetch("/api/posts")]);
</script>
```
