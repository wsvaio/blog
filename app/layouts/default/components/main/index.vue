...
const { showAside = true } = defineProps<{ showAside: boolean; }>();
...
watch(
  ...
  (value, oldValue) => {
    ...
    if (rightDivTop < min) rightDivTop = min;
    if (rightDivTop > max) rightDivTop = max;
  }
);
...
<template>
  <main flex="1" m="x-auto" max="w-[var(--max-width)]" w="full" z="10" pos="relative"
    grid="~ cols-[1fr] md:cols-[3fr_1fr]" gap="1em" p="1em" items="start" box="border" :style="{
      gridTemplateColumns: showAside ? undefined : '1fr',
    }">
    <div flex="~ col" gap="1em" overflow="hidden">
      <slot />
    </div>
    <div v-if="showAside" ref="rightDivRef" flex="~ col" gap="1em" pos="md:sticky" :style="{
      top: `${rightDivTop}px`,
    }">
      <!-- <article-about-card /> -->
      <!-- <tiangou-card /> -->
