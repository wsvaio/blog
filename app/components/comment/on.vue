...
const { execute, error, pending } = useAsyncData(async () => {
  ...
  const result = commentId
    ? await $fetch(`/api/comment/${commentId}/comment`, {
      method: "POST",
      body: {
        ...pick(user, ["avatar", "accept_emails", "email", "name", "site"]),
        ...form,
        articleId,
      },
    })
...
<template>
  <form class="comment-on" @submit.prevent="execute()">
    ...
    <ui-textarea v-model="form.content" placeholder="多想聆听你的声音" :textarea="{ required: 'true' }">
      <!-- <template #placeholder>
        <ui-typewriter :content="message?.content" @finish="nextMessage" />
      </template> -->
    </ui-textarea>
    <!-- <markdown-editor v-model="content" /> -->

    <div grid="~ cols-[max-content_1fr_1fr] rows-3" gap="1em" mt="1em">
      <ui-avatar-input v-model="user.avatar" h="96px" w="96px" grid="row-span-2" :img="{ alt: '头像' }" />
      <ui-input v-model="user.email" placeholder="邮箱(可获取上次登录信息)" :input="{ required: 'true', type: 'email' }"
        grid="col-span-2" @input="handleEmailInput" />

      <ui-input v-model="user.name" placeholder="昵称" :input="{ required: 'true' }" />

      <ui-input v-model="user.site" placeholder="http(s)://主页" />
      ...
      <div grid="col-span-full" flex="~" items="center">
        ...
        <label title="如果有人回复你，将会邮件通知">
          <input v-model="user.accept_emails" type="checkbox" />
          <span>接收邮件</span>
        </label>
        ...
        <ui-awesome-button ml="auto" h="full" w="7em" :disabled="pending">
