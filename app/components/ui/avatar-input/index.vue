...
const { execute: executeAvatar, status } = $(
  useLazyFetch<{ content: string }>("/api/common/pp", {
    onResponse(data) {
      if (data.response.status == 200) modelValue = data.response._data?.content || "";
    },
    immediate: false,
    // server: false,
  })
...
<template>
  <div class="avatar-input">
    <client-only>
      <img :src="modelValue || ''" h="96px" w="96px" grid="row-span-full" alt="头像" @error="executeAvatar()" />
    </client-only>

    <ui-loading :show="status == 'pending'" />
    <div class="control">
      <button
        class="i-material-symbols-light-download"
        title="下载"
        @click.prevent="modelValue && saveAs(modelValue)"
      />
